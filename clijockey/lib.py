...
class CLIMachine(Machine):
    ...
    def after_iter_enable_credentials_cb(self):
        try:
            self.account = next(self.priv_account_iter)
        except:
            # FIXME: Log an out-of-credentials error here...
    ...
    def after_interact_cb(self):
        # We logged in to vty successfully
        self.login_attempts_left = 0
    ...
    def get_next_credentials(self):
        """Iterate over credentials (including retrying the same credentials)"""
        while self.login_attempts_left > 0:

            for account in self.credentials:
                yield account

            ## Decrement login_attempts_left after trying all credentials...
            self.login_attempts_left = self.login_attempts_left - 1
    ...
    def execute(self, line, timeout=-1, wait=0.0, regex="", auto_endline=True,
        ...
        assert isinstance(line, str) or isinstance(line, str)
